# GotitApiJavascriptSdk.REQUESTCREATEVOUCHERLINKE
...
**amount** | **Number** | + Voucher value. Ensures the following conditions: + Is a multiple of 1000 + Is within the price range minimum - maximum value that the partner and Got It have agreed upon. | 
...
**productId** | **Number** | + Product code. Specify the template displayed on the voucher (list of Categories and Brands). + If no transmission will use Got It&#39;s default productId | [optional] 
**useOtp** | **Number** | &#x3D; 1 if the end user is required to enter a password to view the voucher. Example: 1 | 
